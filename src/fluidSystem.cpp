...
FluidSystem::FluidSystem()
{
    ...
    m_smoothRadius = 0.2f;   // 光滑核半径
    m_pointDistance = 0.1f;

    m_rexSize[0] = 0;
    m_rexSize[1] = 0;
    m_rexSize[2] = 0;

    m_boundaryStiffness = 500.0f;
    m_boundaryDampening = 100.0f;
    m_speedLimiting = 0;
...
glm::vec3 FluidSystem::_boundaryForce(Point *p)
{
    ...
    // 边界情况
    //  Z方向边界
    diff = 0.1f * m_unitScale - (p->pos.z - m_sphWallBox.min.z) * m_unitScale;
    if (diff > 0.0f)
    {
    ...
    diff = 0.1f * m_unitScale - (m_sphWallBox.max.z - p->pos.z) * m_unitScale;
    if (diff > 0.0f)
    {
        glm::vec3 norm(0, 0, -1.0);
        float adj = m_boundaryStiffness * diff - m_boundaryDampening * glm::dot(norm, p->velocity);
    ...
    // X方向边界
    diff = 0.1f * m_unitScale - (p->pos.x - m_sphWallBox.min.x) * m_unitScale;
    if (diff > 0.0f)
    {
    ...
    diff = 0.1f * m_unitScale - (m_sphWallBox.max.x - p->pos.x) * m_unitScale;
    if (diff > 0.0f)
    {
        glm::vec3 norm(-1.0, 0, 0);
        float adj = m_boundaryStiffness * diff - m_boundaryDampening * glm::dot(norm, p->velocity);
    ...
    // Y方向边界
    diff = 0.1f * m_unitScale - (p->pos.y - m_sphWallBox.min.y) * m_unitScale;
    if (diff > 0.0f)
    {
    ...
    diff = 0.1f * m_unitScale - (m_sphWallBox.max.y - p->pos.y) * m_unitScale;
    if (diff > 0.0f)
    {
        glm::vec3 norm(0, -1.0, 0);
        float adj = m_boundaryStiffness * diff - m_boundaryDampening * glm::dot(norm, p->velocity);
...
void FluidSystem::updatePosAndVel()
{
    for (int i = 0; i < m_pointBuffer.size(); i++)
    {
        ...
        // if (pi->pos.x < m_sphWallBox.min.x)
        //     pi->pos.x = m_sphWallBox.min.x;
        // if (pi->pos.y < m_sphWallBox.min.y)
        //     pi->pos.y = m_sphWallBox.min.y;
        // if (pi->pos.z < m_sphWallBox.min.z)
        //     pi->pos.z = m_sphWallBox.min.z;
        // if (pi->pos.x > m_sphWallBox.max.x)
        //     pi->pos.x = m_sphWallBox.max.x;
        // if (pi->pos.y > m_sphWallBox.max.y)
        //     pi->pos.y = m_sphWallBox.max.y;
        // if (pi->pos.z > m_sphWallBox.max.z)
        //     pi->pos.z = m_sphWallBox.max.z;
        // 弹入位置数据
